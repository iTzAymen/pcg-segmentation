...
os.makedirs('PhysioNetCycles/training', exist_ok=True)
os.makedirs('PhysioNetCycles/validation', exist_ok=True)
j = 0
for row in df.itertuples():
    ...
    for i, c_sig in enumerate(c_signals):
        ...
        if 'validation' in row.relative_path:
            savepath = 'PhysioNetCycles/validation/' + file_name
        else:
            savepath = 'PhysioNetCycles/training/' + file_name
...
reference_df.to_csv('PhysioNetCycles/REFERENCE.csv')